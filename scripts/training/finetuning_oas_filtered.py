...
# These can stay if you still want to keep them around for reporting/upstream provenance,
# but they are no longer used to get OAS H3 indices.
OAS_PLDDT_DIRS = [
    "/home/alanwu/Documents/iggen_model/data/training_data/oas data/plddt_table/1279052+1279060",
    "/home/alanwu/Documents/iggen_model/data/training_data/oas data/plddt_table/1287155_part1",
    "/home/alanwu/Documents/iggen_model/data/training_data/oas data/plddt_table/1287155_part2",
]

# NEW: OAS CDR loop fasta files
OAS_CDR_FASTAS = [
    "/home/alanwu/Documents/iggen_model/data/oas data/cdr_sequence/1279052/cdr_loops.fasta",
    "/home/alanwu/Documents/iggen_model/data/oas data/cdr_sequence/1279060/cdr_loops.fasta",
    "/home/alanwu/Documents/iggen_model/data/oas data/cdr_sequence/1287155/cdr_loops.fasta",
]

OUT_DIR = "/home/alanwu/Documents/iggen_model/piggen_lora_cdr_masked_prefiltered_random"
...
def is_sabdab_header_id(header_id: str) -> bool:
    base = normalize_header_id(header_id)
    return base.lower().endswith(".pdb")
...
# ----------------------------
# NEW: read OAS CDR loop fasta and collect exact H3 loop sequences
# ----------------------------

def read_oas_h3_sequences_from_cdr_fastas(paths: list[str]) -> dict[str, str]:
    """
    Reads records like:
      >AA...contig_1__AA...contig_2|CDRH3
      ARDG...
    and returns:
      canonical_pair_key -> exact CDRH3 sequence

    Also stores reversed contig-order alias so either order can match.
    """
    out = {}
...
def _store_oas_h3_record(header: str, seq: str, out: dict[str, str]):
    """
    Only keep CDRH3 records.
    Header example:
      AAAC..._contig_1__AAAC..._contig_2|CDRH3
    """
    if "|" not in header:
        return
...
def find_unique_subsequence_span(full_seq: str, sub_seq: str):
    ...
    if second != -1:
        return None  # ambiguous exact repeat
...
def build_loss_mask_from_exact_span(fv_len: int, start: int, end: int) -> torch.Tensor:
    """
    Generic exact-span mask builder.
    """
    m = torch.zeros(fv_len, dtype=torch.float32)
...
class FvMaskedDataset(Dataset):
    ...
    For each example:
      ...
      - OAS H3 spans are found by searching the exact CDRH3 sequence
        from OAS cdr_loops.fasta inside the full Fv sequence
    """
    def __init__(
        self,
        ...
        self.items = []
        self.oas_h3_span_by_header = {}

        sab_total = sab_ok = sab_missing = 0
        oas_total = oas_ok = oas_missing = oas_no_h3 = oas_not_found = oas_ambig = 0

        for hid in self.fasta_map.keys():
            hid_str = str(hid)

            if is_sabdab_header_id(hid_str):
                sab_total += 1
                ...
                if span is None:
                    n_matches = full_seq.count(h3_seq)
                    if n_matches == 0:
                        oas_not_found += 1
                    else:
                        oas_ambig += 1
                ...
                s, e = span
                self.oas_h3_span_by_header[hid_str] = (s, e)
                oas_ok += 1
    ...
    def __getitem__(self, idx):
        ...
        if source == "sabdab":
            ...
            if header_id not in self.oas_h3_span_by_header:
                raise ValueError(f"No cached OAS H3 span for {header_id}")

            s, e = self.oas_h3_span_by_header[header_id]
            loss_mask = build_loss_mask_from_exact_span(
                fv_len=fv_len,
                start=s,
                end=e,
            )
...
def main():
    ...
    for _, row in df_sabdab.iterrows():
        hid = normalize_header_id(row["fasta_header_id"])
        if hid not in sabdab_df_by_header:
            sabdab_df_by_header[hid] = row

    # ----------------------------
    # Load exact OAS H3 loop sequences from cdr_loops.fasta
    # ----------------------------
    ...
    # Report composition
    n_train_sab = sum(1 for k in train_ids if is_sabdab_header_id(k))
    n_val_sab = sum(1 for k in val_ids if is_sabdab_header_id(k))
